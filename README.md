...
迁移自原脚本项目 https://github.com/janmk1453/deepseek-tavern-script 

## 预览

> 扩展主面板为全屏独立页（`#aus-panel`），左侧可收起导航，右侧多视图切换，样式对齐 DeepSeek。
...
- **可视化**：多图（`Token/费用/命中率/请求数/耗时速率/模型占比` 等）+ 热力图 + 主图标（`Y` 8 选 × `X` 5 维度，双轴可读）
...
- **八小块**：每轮费用 `CNY`/每轮 `Token`/平均耗时 `s`/输出速率 `t/s`
