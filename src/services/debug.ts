...
export function generateDebugBatch() {
  ...
  for (let d = 0; d < totalDays && generated < count; d++) {
    const curDate = new Date(startDate); curDate.setDate(startDate.getDate() + d);
    for (let i = 0; i < perDay && generated < count; i++) {
      ...
      const total = h + m + o;
      const ts = new Date(curDate); ts.setHours(Math.floor(Math.random()*24), Math.floor(Math.random()*60), Math.floor(Math.random()*60), 0);
