...
function peakHoursFor(seg, settings2) {
  if (seg?.peakHours?.length) return seg.peakHours;
  return settings2 && settings2.peakHours || DEFAULT_PEAK_HOURS;
}
function hasCustomForModel(model, settings2) {
  const raw = model || "deepseek-v4-flash";
  const m = normalizeModel(raw);
  for (const cm of settings2.customModels || []) if (matchCustom(cm, m, raw)) return true;
  return false;
}
function effectivePricingFor(model, uTs, settings2, base) {
  const m = normalizeModel(model || "deepseek-v4-flash");
  if (hasCustomForModel(model, settings2)) return base;
...
function calcCost(u, settings2) {
  ...
  const useNewPricing = settings2.useNewPricing && u.timestamp >= settings2.newPricingDate;
  let p;
  let priceType;
...
const repository = {
  ...
  async hydrate() {
    ...
    if (!state$2.settings.pricingSync) {
      state$2.settings.pricingSync = { enabled: false, mode: "add-missing", exchangeRate: 7.2, useLiveRate: true, autoIntervalHours: 0, lastSync: null, lastRateFetch: null, recalcOnSync: false, showSyncedModels: false, syncedMarkVersion: 0 };
      try {
        saveHot({ settings: state$2.settings });
      } catch {
      }
      ...
      try {
        const def = { enabled: false, mode: "add-missing", exchangeRate: 7.2, useLiveRate: true, autoIntervalHours: 0, lastSync: null, lastRateFetch: null, recalcOnSync: false, showSyncedModels: false, syncedMarkVersion: 0 };
        const ps = state$2.settings.pricingSync;
...
function applyImportedData(d, mode) {
  ...
  try {
    ...
    const patch = { total_tokens: tt, total_cost: tc, input_tokens: it, output_tokens: ot, cache_hit_tokens: ch, cache_miss_tokens: cm, input_cost: ic, output_cost: oc };
    if (mode === "merge") patch.rounds = (state$2.rounds || 0) + addedOfficial;
...
function priceClose(a, b, tol = 0.05) {
...
async function markLegacySyncedModels(opts) {
  ...
  if (marked) {
    ...
    if (!opts?.skipRerender) {
      try {
        const { renderSettings: renderSettings2 } = await Promise.resolve().then(() => settings);
        const doc = window.parent?.document ?? document;
        if (doc?.getElementById("aus-settings")) renderSettings2(doc);
      } catch {
      }
    }
  } else if (catalog) {
...
let ySelected = /* @__PURE__ */ new Set(["total_token"]);
let xSelected = "day";
    ...
    <div id="aus-sidebar" style="width:220px;flex-shrink:0;background:var(--ds-sidebar-bg);border-right:1px solid var(--ds-border);display:flex;flex-direction:column;overflow:hidden;">
      <div style="height:56px;display:flex;align-items:center;justify-content:space-between;padding:0 14px;flex-shrink:0;">
        <div style="display:flex;flex-direction:column;min-width:0;" id="aus-brand">
          ...
          <span style="font-size:11px;color:var(--ds-text-2);white-space:nowrap;">v${"3.0.6"}</span>
        </div>
            ...
            <div style="display:grid;gap:12px;">
              ...
              <div class="ds-card" style="display:grid;gap:8px;">
                ...
                <div style="display:flex;gap:8px;align-items:center;">
                  ...
                  <span style="font-size:11px;color:var(--ds-text-3);">当前 v${"3.0.6"} · 每 6 小时自动检查</span>
                </div>
              </div>
            </div>
          </div>
  ...
  try {
    ...
    if (updBtn) {
      updBtn.onclick = () => {
        ...
        import("./update-fXu93OrG.js").then((m) => m.checkUpdate(true).finally(() => {
...
function openPanel() {
  ...
  refreshUI();
  try {
    import("./update-fXu93OrG.js").then((m) => m.maybeAutoCheck());
  } catch {
  }
}
function closePanel() {
...
async function init() {
  ...
  const mount = () => {
    ...
    try {
      Promise.resolve().then(() => pricingSync).then((m) => m.markLegacySyncedModels?.()).catch(() => {
      });
    } catch {
    }
  };
