...
function fillDebugModelSelect(doc) {
  ...
  if (!sel) return;
  const models = Object.keys(PRICING).concat((state$2.settings.customModels || []).map((c) => c.model).filter(Boolean)).filter((m) => HIDDEN_PRICING_MODELS.indexOf(m) === -1);
  ...
  try {
    ...
    if (updBtn) {
      updBtn.onclick = () => {
        ...
        import("./update-DuWlAHZq.js").then((m) => m.checkUpdate(true).finally(() => {
...
function openPanel() {
  ...
  refreshUI();
  try {
    import("./update-DuWlAHZq.js").then((m) => m.maybeAutoCheck());
  } catch {
  }
}
function closePanel() {
